...
To counter disparate impact, we wish to enforce \emph{demographic parity}, which demands that the distribution of scores for any protected classes is the same.  Let $\hy$ be a model's prediction.  Formally, demographic parity is defined as:
\begin{align}
p(\hy = 1 \given \bd z = k_1) = p(\hy = 1 \given \bd z = k_2),
\end{align}  
...
\begin{align}
\min \left\{\frac{p(\hy = 1 \given \bd z = k_1)}{p(\hy = 1 \given \bd z = k_2)}, \frac{p(\hy = 1 \given \bd z = k_2)}{p(\hy = 1 \given \bd z = k_1)} \right\} \geq \frac{q}{100}
\end{align} 
where $q = 80$ in the legal definition.  Recently, Hu and Chen \yrcite{hu2018short} additionally argue that short-term enforcement of demographic parity has long-term benefits for countering discrimination against minorities in the labor market.  
...
&+ \lambda \cdot \left | \frac{1}{N}  \sum_{n=1}^N (\bidx z n - \bd \hz) \cdot \idx d n \right|, \nonumber
\end{align}
...
\section{Results}
Our empirical results are evaluated on the adult income dataset.  We first naively train a vanilla neural network and show how it suffers from disparate impact.  Then, we apply our methods for enforcing demographic parity to exhibit how this disparate impact can be mitigated.  
